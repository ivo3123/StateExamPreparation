...
\subsection{Механизъм на динамичното свързване (vtable / vptr)}

За всеки клас с поне една виртуална функция компилаторът генерира \textbf{виртуална таблица (vtable)} -- масив от указатели към конкретните виртуални функции. Всеки обект от такъв клас съдържа скрит \textbf{указател към vtable (vptr)}, инициализиран от конструктора.

При извикване \texttt{p->f()} (където \texttt{f} е виртуална) компилаторът генерира код:
\begin{enumerate}
    \item Прочети \texttt{vptr} от обекта.
    \item Намери слота на \texttt{f} във \texttt{vtable}.
    \item Извикай функцията на намерения адрес.
\end{enumerate}

\begin{lstlisting}
// Conceptual memory layout of an object with virtual functions:
//
//  [ vptr ] --> vtable of actual class:
//  [ data ]       [0] -> &ActualClass::f
//                 [1] -> &ActualClass::g
//                 ...
\end{lstlisting}

\textbf{Цена:} Всяко извикване на виртуална функция добавя едно индиректно разкостване (разглеждане на vptr и vtable). Всеки обект с виртуални функции носи допълнителните \texttt{sizeof(void*)} байта за vptr.
...
Widget arr[5];   // calls default constructor 5 times
...
\begin{lstlisting}
Widget* arr = new Widget[5];  // calls default constructor 5 times
...
\begin{lstlisting}
Widget* arr[5];               // array of 5 pointers (uninitialized)
arr[0] = new Widget(42);      // each element independently constructed
arr[1] = new Widget(7);
// ...
...
\textbf{Задължително:} Деструкторът на базовия клас трябва да е \texttt{virtual}, за да се извикат правилните деструктори на наследниците при \texttt{delete zoo[i]}.
