...
\author{Ивайло Андреев \\+ chat gpt 5.5, DeepSeek-V3, gemini 3 Flash, Claude Sonnet 4.6}
\date{9 май 2026}
...
\begin{definition}
\textbf{\textit{Суперключ}} е множество от атрибути, което функционално определя всички останали атрибути на релацията (не е задължително минимално).
\end{definition}

\begin{definition}
\textbf{\textit{Кандидат-ключ (КК)}} е минимален суперключ. Една релация може да има повече от един кандидат-ключ.
\end{definition}

\begin{definition}
\textbf{\textit{Първичен ключ (ПК)}} е избран кандидат-ключ, използван за идентификация на кортежите. Стойностите му не могат да бъдат \texttt{NULL}.
\end{definition}
...
Дадена е схема Student(student\_id, name, dept\_id, dept\_name, advisor\_id) с ФЗ:
\begin{itemize}
    \item $\text{student\_id} \rightarrow \text{name}, \text{dept\_id}, \text{advisor\_id}$
    \item $\text{dept\_id} \rightarrow \text{dept\_name}$
\end{itemize}

По \textbf{транзитивност}: $\text{student\_id} \rightarrow \text{dept\_id}$ и $\text{dept\_id} \rightarrow \text{dept\_name}$, следователно $\text{student\_id} \rightarrow \text{dept\_name}$.

По \textbf{разлагане}: от $\text{student\_id} \rightarrow \text{name}, \text{dept\_id}, \text{advisor\_id}$ следват отделно $\text{student\_id} \rightarrow \text{name}$, $\text{student\_id} \rightarrow \text{dept\_id}$, $\text{student\_id} \rightarrow \text{advisor\_id}$.

По \textbf{комбинация}: обратно, трите отделни зависимости се комбинират обратно в $\text{student\_id} \rightarrow \text{name}, \text{dept\_id}, \text{advisor\_id}$.
...
\begin{itemize}
    \item \textbf{Излишества} --- Повторение на информация.

    \textit{Пример:} Ако таблица съхранява информация за служителя и неговия отдел, информацията за отдела се повтаря за всеки служител в него.
    ...
    \textit{Пример:} Таблица Employee(emp\_id, name, dept\_id, dept\_name) без декомпозиция. Отдел 10 се казва „Маркетинг“. Ако преименуваме отдела на „Дигитален маркетинг“, трябва да обновим колоната dept\_name за \emph{всеки} служител в отдел 10. Ако пропуснем дори един ред, базата данни е в неконсистентно състояние --- различни редове твърдят различни имена за един и същ отдел.

    \item \textbf{Аномалии при добавяне} --- Невъзможност да добавим определена информация, без да добавим и друга.

    \textit{Пример:} Не можем да добавим нов отдел, ако информацията за отдела се пази само заедно със служители.

    \item \textbf{Аномалии при изтриване} --- Изтриването на данни води до загуба на друга важна информация.

    \textit{Пример:} Изтриването на последния служител от даден отдел може да доведе и до загуба на информацията за самия отдел, ако тя се пази в същата таблица.
\end{itemize}
...
\textit{Пример на нарушение:} Таблица Student(student\_id, name, phone\_numbers), където phone\_numbers съдържа стойности от вида \texttt{"0888111222, 0899333444"} --- списък от телефони в един атрибут. Това нарушава 1НФ, защото стойността не е атомарна. Решението е или да се добави отделна таблица StudentPhone(student\_id, phone), или да се добавят отделни колони phone1, phone2 (ако броят е фиксиран).

Друг пример: атрибут address, съдържащ \texttt{"ул. Витоша 15, София 1000"} --- ако приложението трябва да търси по град, адресът трябва да се раздели на отделни атомарни колони (street, city, zip).
...
\textit{Класически пример на 3НФ, но не НФБК:} Таблица CourseTeacher(student, course, teacher), където:
\begin{itemize}
    \item Всеки студент записва курс при точно един преподавател: $\{\text{student}, \text{course}\} \rightarrow \text{teacher}$
    \item Всеки преподавател води само един курс: $\text{teacher} \rightarrow \text{course}$
\end{itemize}
Кандидат-ключовете са $\{\text{student}, \text{course}\}$ и $\{\text{student}, \text{teacher}\}$. Атрибутът course е \emph{prime} (участва в КК), затова ФЗ $\text{teacher} \rightarrow \text{course}$ не нарушава 3НФ. Но нарушава НФБК, защото teacher не е суперключ.

Това е илюстрация на факта, че \textbf{разликата между 3НФ и НФБК се проявява само при наличие на припокриващи се кандидат-ключове} --- ситуация, която на практика е сравнително рядка.
...
\section{Алгоритъм за разложение на релация в НФБК [Опционално]}
...
$R_1 = C^+ = \{A, C, D\}$, $\quad R_2 = (R \setminus C^+) \cup \{C\} = (\{A,B,C,D\} \setminus \{A,C,D\}) \cup \{C\} = \{B, C\}$

Разложение:
\begin{itemize}
    \item $R_1(A, C, D)$ --- проекция на ФЗ: $C \rightarrow D$, $D \rightarrow A$
    \item $R_2(B, C)$ --- нито една нетривиална ФЗ
\end{itemize}

В $R_1(A, C, D)$ кандидат-ключ е само $\{C\}$. ФЗ $D \rightarrow A$ нарушава НФБК (защото $D$ не е суперключ в $R_1$).
