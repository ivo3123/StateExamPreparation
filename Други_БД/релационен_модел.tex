...
\begin{definition}
\textbf{\textit{Проекция}} $\pi_{A_{i_1}, \ldots, A_{i_k}}(R)$ на релация $R$ със атрибути $\{A_1, \ldots, A_n\}$ е релация, съдържаща само избраните атрибути $\{A_{i_1}, \ldots, A_{i_k}\}$.
\end{definition}

Е еквивалентна на SQL SELECT с определени колони.
...
\begin{definition}
\textbf{\textit{Частното}} $R \div S$ (където $S$ има подмножество на атрибутите на $R$) съдържа всички кортежи от $\pi_{\text{остатък}}(R)$, за които всяка комбинация с кортежи от $S$ съществува в $R$.
\end{definition}

Интуиция: Ако $R$ съдържа (student\_id, sport) и $S$ съдържа (sport), то $R \div S$ дава студентите, които правят ВСЕ спортове в $S$.
