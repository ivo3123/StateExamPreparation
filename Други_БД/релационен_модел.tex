...
Нерелационните модели (NoSQL) съществуват като алтернатива и включват документни БД (единицата е JSON/BSON документ — например MongoDB), кей-стойностни схранилища (например Redis), графови БД (например Neo4j) и колонни БД (например Cassandra). Те са подходящи за големи обеми данни, хоризонтално мащабиране и гъвкави схеми, но в тази тема фокусът е върху релационния модел.
...
\textit{Пример на неатомарни стойности:} Атрибут \texttt{phone\_numbers} със стойност \texttt{"0888111222, 0899333444"} нарушава атомарността --- един атрибут съдържа множество от стойности. Правилното решение е отделна таблица StudentPhone(student\_id, phone) или отделни колони phone1, phone2.
...
\begin{definition}
\textbf{\textit{Екземпляр на релация}} е множеството от всички текущо съхранени кортежи — динамична характеристика, която се променя при всека INSERT/UPDATE/DELETE.
\end{definition}

Свързани понятия:

\begin{itemize}
    \item \textbf{Степен (Арити)} --- Броят на атрибутите в схемата. Статична характеристика.
    \item \textbf{Кардиналност} --- Броят на кортежи в екземпляра в даден момент. Динамична характеристика.
\end{itemize}
...
\begin{definition}
\textbf{\textit{Суперключ}} е множество от атрибути, което еднозначно определя всеки кортеж в релацията (не е задължително минимално).
\end{definition}

\begin{definition}
\textbf{\textit{Кандидат-ключ}} е минимален суперключ. Една релация може да има повече от един кандидат-ключ.
\end{definition}

\begin{definition}
\textbf{\textit{Първичен ключ}} е избран кандидат-ключ, използван за идентификация на кортежите. Стойностите му не могат да бъдат \texttt{NULL}.
\end{definition}
